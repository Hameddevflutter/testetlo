...
import 'package:testetlo/features/home_screen/logic/controller/select_time_controller.dart';
...
class AboutConsultantPage extends StatelessWidget {
  ...
  @override
  Widget build(final BuildContext context) => GetBuilder<SelectTimeController>(
        init: SelectTimeController(),
        builder: (final controller) => Stack(
          children: [
            ListView.builder(
              itemCount: controller.weekDays.length,
              itemBuilder: (final context, final index) => Obx(
                () => SelectDayUiWidget(
                  checkBoxValue: controller.selectDayStatus[index],
                  checkBoxOnChange: (final value) {
                    controller.selectDayStatus[index] = value!;
                    if (value) {
                      controller.selectedDaysValues.add(
                        controller.weekDays.entries.toList()[index].value,
                      );
                    } else {
                      controller.selectedDaysValues.remove(
                        controller.weekDays.entries.toList()[index].value,
                      );
                    }
                  },
                ...
                child: Row(
                  ...
                  children: [
                    MyButton(
                      ...
                      onTap: () {
                        controller
                            .saveSelectedDays(controller.selectedDaysValues);
                      },
                    ),
                    MyButton(
                      onTap: () {},
